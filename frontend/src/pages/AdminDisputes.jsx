...
export default function AdminDisputes() {
  ...
  useEffect(() => { load(); }, []);
  async function resolve(escrow, buyerAmount) {
    try { setBusy(escrow.escrowId); setError("");
      ...
      await api.post(`/admin/disputes/${escrow.escrowId}/resolved`, { wallet, buyerAmount, transactionHash });
      load();
...
function Case({ escrow, wallet, busy, resolve }) {
  ...
  return <article className="rounded-2xl border border-red-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-2xl font-bold">{escrow.escrowId}</h2><p className="text-slate-600">{escrow.amount} {escrow.assetSymbol} · {dispute.status}</p></div><span className="rounded-full bg-red-100 px-3 py-1 font-bold text-red-700">{dispute.reason}</span></div><div className="mt-5 grid gap-4 md:grid-cols-2"><section><h3 className="font-bold">{dispute.openedBySide} claim</h3><p className="mt-2 whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-sm">{dispute.statement}</p></section>{(dispute.responses || []).map((response, index) => <section key={index}><h3 className="font-bold">{response.side} response</h3><p className="mt-2 whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-sm">{response.statement}</p></section>)}</div><h3 className="mt-5 font-bold">Private evidence</h3><ul className="mt-2 text-sm">{evidence.map((file) => <li key={file.id}><a className="text-blue-700 underline" target="_blank" rel="noreferrer" href={`${API_BASE_URL}/escrow/${escrow.escrowId}/dispute/evidence/${file.id}?wallet=${encodeURIComponent(wallet)}`}>{file.side}: {file.name}</a></li>)}</ul><div className="mt-6 rounded-xl border border-slate-200 p-4"><label className="block text-sm font-bold">Buyer refund amount ({escrow.assetSymbol})<input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min="0" max={escrow.amount} step="any" className="mt-2 w-full rounded-lg border p-2" /></label><div className="mt-3 grid gap-3 sm:grid-cols-3"><button disabled={busy} onClick={() => resolve(escrow, escrow.amount)} className="rounded-lg bg-blue-600 p-3 font-semibold text-white disabled:opacity-50">Refund buyer</button><button disabled={busy} onClick={() => resolve(escrow, 0)} className="rounded-lg bg-green-600 p-3 font-semibold text-white disabled:opacity-50">Pay seller</button><button disabled={busy} onClick={() => resolve(escrow, amount)} className="rounded-lg bg-amber-500 p-3 font-semibold text-white disabled:opacity-50">{busy ? "Resolving..." : "Split funds"}</button></div></div></article>;
}
