    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-4xl font-bold text-slate-900">{isSellerRole ? "Pending Sales" : "Pending Orders"}</h1>
        ...
        <div className="mt-8 space-y-5">
          ...
          {orders.map((order) => (
            <article key={order.escrowId} className="rounded-3xl border border-orange-200 bg-white p-7 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
...
function EmptyState({ icon, title, message }) {
  return <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center"><div className="text-5xl">{icon}</div><h2 className="mt-5 text-2xl font-bold text-slate-900">{title}</h2><p className="mt-2 text-slate-600">{message}</p></div>;
}
